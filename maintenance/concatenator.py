...
def concatenate(table, assembly, source_path, target_path):
    ...
    for experiment in table:
        basename = os.path.join(target_path,"{0}_{1}_all".format(experiment, assembly))
        filename_json = basename + '.json'
        filename_bed  = basename + '.bed'

        # collect sites count from each json file
        filenames = map(lambda j: os.path.join(source_path,j['id']+'.json'), table[experiment])
        sites = 0
        ...
        filenames = map(lambda j: os.path.join(source_path,j['id']+'.bed'), table[experiment])
        with open(filename_bed, "w") as f:
            for bed_file in filenames:
                out = subprocess.check_output([bedtools,'bed12tobed6','-i',bed_file])
                f.write(out)
                f.flush()
        ...
        json_contents = [
            { "id": "{0}_{1}_all".format(experiment, assembly),
              ...
              "summary": "all {0} target sites ({1})".format(experiment, assembly),
              "description": "",
              "methods": "",
              "references": "",
              "sites": sites
...
def walk_tree():
    ...
    for species in os.listdir(REGULATORS_PATH):
        ...
        for genome in os.listdir(species_path):
            ...
            # for each of the experiments, create a concatenated json file containing all data sets
