...
@patch('webdorina.app.Redis', fakeredis.FakeRedis)
...
chr1	doRiNA2	gene	2001	3000	.	+	.	ID=gene01.02	chr1	2350	2360	PARCLIP#scifi*scifi_intron	6	+"""
        ...
        app.run_analyse(self.data_dir, 'results:fake_key',
                        'results:fake_key_pending', query,
                        'fake-uuid')
    ...
    def test_run_analyse_custom_regulator(self):
        ...
        session_store = app.config['SESSION_STORE'].format(
            unique_id='fake-uuid')
        expected_trace = '''Called run.run.Dorina.analyse(
    genome='hg19',
    match_a='any',
...
chr1	doRiNA2	gene	2001	3000	.	+	.	ID=gene01.02	chr1	2350	2360	PARCLIP#scifi*scifi_intron	6	+"""
    ...
    def test_filter(self):
        ...
        app.filter_genes(['gene01.01', 'gene01.02'],
                                   'results:fake_full_key',
                                   'results:fake_key',
                                   'results:fake_key_pending', 'fake-uuid')
...
class DorinaTestCase(TestCase):
    ...
    @patch('webdorina.app.redis_store ')
    def setUp(self):
        self.maxDiff = None
        self.tt = TraceTracker()
        datadir = os.path.join(os.path.dirname(__file__), 'data')
        = RedisStore('fake_store', self.tt)
...
Called fake_store.llen(
    ...
    def test_search_filtered_full_results_cached(self):
        """Test search() with filter_genes and full results in cache"""
